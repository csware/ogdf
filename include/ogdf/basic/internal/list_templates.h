...
inline void safeForEach(CONTAINER& container,
 ...
 */
template<typename CONTAINER>
inline bool safeTestForEach(CONTAINER& container,
...
ITERATOR chooseIteratorByFastTest(CONTAINER& container,
 ...
 */
template<typename CONTAINER, typename TYPE, typename ITERATOR>
ITERATOR chooseIteratorBySlowTest(CONTAINER& container,
		std::function<bool(const TYPE&)> includeElement, int size) {
 ...
 */
template<typename CONTAINER, typename TYPE, typename ITERATOR>
ITERATOR chooseIteratorFrom(CONTAINER& container, std::function<bool(const TYPE&)> includeElement,
		bool isFastTest) {
	...
	if (size > 0) {
		...
		if (!includeElement(*result)) {
			if (isFastTest) {
				result = chooseIteratorByFastTest<CONTAINER, TYPE, ITERATOR>(container,
						includeElement);
			} else {
				result = chooseIteratorBySlowTest<CONTAINER, TYPE, ITERATOR>(container,
						includeElement, size);
			}
		}
	}
...
typename CONTAINER::iterator chooseIteratorFrom(
	...
	return internal::chooseIteratorFrom<CONTAINER, TYPE, typename CONTAINER::iterator>(container,
			includeElement, isFastTest);
}
...
typename CONTAINER::const_iterator chooseIteratorFrom(
		...
		std::function<bool(const TYPE&)> includeElement = [](const TYPE&) { return true; },
		bool isFastTest = true) {
	return internal::chooseIteratorFrom<const CONTAINER, TYPE, typename CONTAINER::const_iterator>(
			container, includeElement, isFastTest);
}
