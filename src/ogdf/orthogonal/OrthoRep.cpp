...
// constructor
OrthoRep::OrthoRep(CombinatorialEmbedding& E) : m_pE(&E), m_angle(E, 0), m_bends(E) {
	m_preprocess = true;
	m_pattern2 = true;
}
		...
		for (adjEntry adj : f->entries) {
			...
			for (size_t i = 0; i < bs.size(); ++i) {
				...
				case '1':
					...
					break;
				default:
					oss << "bend string of adjacency entry " << adj->index()
	...
	for (face f : E.faces) {
		...
		for (it = faceCycle.begin(); faceCycle.size() > 4 && it.valid(); ++it) {
			if (m_angle[*it] == 1 && m_angle[*faceCycle.cyclicPred(it)] == 1) {
				...
				ListIterator<adjEntry> itBack = faceCycle.cyclicPred(faceCycle.cyclicPred(it));
				...
				while (it != itBack) {
					...
					ListIterator<adjEntry> itBackSucc = faceCycle.cyclicSucc(itBack);
	...
	for (face f : E.faces) {
		...
		while (change) {
...
#endif
				//check if possible
				// pattern defining edges:
				ListIterator<adjEntry> itEnd, itStart, it1one, it1two, it1three, it1four;
				//take care of prit, it will be deleted
				// go clockwise around face!?
				for (prit = faceCycle.begin(); prit.valid() && (faceCycle.size() > 7); ++prit) {
					itEnd = prit; //search pattern backwards
					if (m_angle[*itEnd] != 3) {
						continue;
					}
					it1four = faceCycle.cyclicPred(itEnd);
					if (m_angle[*it1four] != 1) {
						continue;
					}
					it1three = faceCycle.cyclicPred(it1four);
					if (m_angle[*it1three] != 1) {
						continue;
					}
					...
					if (((m_adjExternal == (*itEnd)) || (m_adjExternal == (*it1four))
								|| (m_adjExternal == (*it1three)) || (m_adjExternal == (*it1two))
								|| (m_adjExternal == (*it1one)) || (m_adjExternal == (*itStart)))
							|| ((m_adjAlign == (*itEnd)) || (m_adjAlign == (*it1four))
									|| (m_adjAlign == (*it1three)) || (m_adjAlign == (*it1two))
									|| (m_adjAlign == (*it1one)) || (m_adjAlign == (*itStart)))) {
					...
					//alignment part
					if (PG != nullptr
							&& PG->typeOf((*it1two)->theEdge()->source())
									== Graph::NodeType::generalizationExpander
							&& PG->typeOf((*it1two)->theEdge()->target())
									== Graph::NodeType::generalizationExpander) {
			...
			if (m_pattern2) {
				...
				//check if possible
				// pattern defining edges:
				ListIterator<adjEntry> it1Top, itTopSucc, it1Back, it1Base, it3Start;
				//take care of prit, it will be deleted
				savenext = faceCycle.begin();
				//go clockwise around face!?
				for (prit = faceCycle.begin();
						prit.valid() && savenext.valid() && faceCycle.size() > 6; prit = savenext) {
					++savenext;
					...
					if (m_angle[*it1Back] != 1) {
						continue;
					}
					it1Base = faceCycle.cyclicPred(it1Back);
					if (m_angle[*it1Base] != 1) {
						continue;
					}
					it3Start = faceCycle.cyclicPred(it1Base);
					if (m_angle[*it3Start] != 3) {
						continue;
					}

					//first version
					if (m_angle[*itTopSucc] < 2) {
						continue;
					}
		...
		if (PG != nullptr) {
			...
			//go clockwise around face!?
			for (prit = faceCycle.begin(); prit.valid() && savenext.valid() && faceCycle.size() > 5;
					prit = savenext) {
				++savenext;
...
#if 0
				if (m_angle[*itEnd]    != 3) {
					continue; //nur in einer Richtung
				}
#endif
				...
				if ((ov2->degree() != 1) && (ov->degree() != 1)) {
					continue;
				}
#if 0
				if (PG->typeOf((*itEar)->theEdge()) == -1) {
					continue;
				}
#endif
				...
				if (m_angle[adjHeadSucc] != 1) {
					...
					if (m_angle[adjHeadSucc] == 1) {
						...
						faceCycle.del(itHead);
					}
					if (m_angle[adjHeadSucc] == 2) {
						faceCycle.del(itEnd); //it must have been in list
					}
				} else {
					//split edge
					a1 = m_angle[adHead];
					...
					if (PG) {
						savetype = PG->typeOf(adHead->theEdge());
					}
					edge se = E.split(adHead->theEdge());
					if (PG) {
						PG->typeOf(se) = savetype;
					}
					adjHeadSucc = adHead->faceCycleSucc();
					...
					if (wasDissected) {
						m_dissectionEdge[se] = true;
					}
					if (wasAlign) {
						m_alignmentEdge[se] = true;
					}
			...
			if (m_angle[*it] == 1 && m_angle[*faceCycle.cyclicPred(it)] == 1) {
				...
				while ((it != itBack) && (faceCycle.size() > 4)) {
...
#if 0
					if (savevertex) {
						OGDF_ASSERT(adjSplitSucc == *itsucc);//doesnt work with 180 degree
					}
#endif
					// and close a rectangular face
					edge eDissect = E.splitFace(*itBack, adjSplitSucc);
					...
					if (savevertex && (earSlope == 2)) {
						m_angle[adjSplitSucc->cyclicPred()] = ???
					}
#endif
...
void OrthoRep::gridDissect(PlanRep* PG) {
	...
	for (face f : E.faces) {
		...
		if (PG != nullptr) {
			...
			//go clockwise around face!?
			for (prit = faceCycle.begin();
					prit.valid() && savenext.valid() && (faceCycle.size() > 5); prit = savenext) {
				++savenext;
				//es bleibt herauszufinden, in welcher reihenfolge die kanten ohnehin korrekt
				//durchlaufen werden, dann die andere nehmen
				itEnd = prit; //search pattern backwards
...
void OrthoRep::undissect(bool align) //default false
{
	...
	for (e = G.firstEdge(); e != nullptr; e = eSucc) {
		eSucc = e->succ();
		if (m_dissectionEdge[e] && !(align && m_alignmentEdge[e])) {
	...
	if (align && (m_adjAlign != nullptr)) {
		...
		m_pE->setExternalFace(m_pE->rightFace(m_adjExternal));
	}
...
void OrthoRep::orientate() { orientate(m_pE->getGraph().firstEdge()->adjSource(), OrthoDir::West); }
...
void OrthoRep::orientate(adjEntry adj, OrthoDir dir) {
	...
	OGDF_ASSERT(dir == OrthoDir::East || dir == OrthoDir::West || dir == OrthoDir::North
			|| dir == OrthoDir::South);
...
void OrthoRep::rotate(int r) {
	...
	for (edge e : G.edges) {
		m_dir[e->adjSource()] =
				static_cast<OrthoDir>((static_cast<int>(m_dir[e->adjSource()]) + r) & 3);
		m_dir[e->adjTarget()] =
				static_cast<OrthoDir>((static_cast<int>(m_dir[e->adjTarget()]) + r) & 3);
	}
}

// computes further information about cages which are collected in class
// VertexInfoUML
void OrthoRep::computeCageInfoUML(const PlanRep& PG) {
