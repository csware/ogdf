...
struct OGDF_EXPORT GridPointInfo {
	...
	bool operator==(const GridPointInfo& i) const { return m_v == i.m_v && m_e == i.m_e; }

	bool operator!=(const GridPointInfo& i) const { return !operator==(i); }
...
public:
	int hash(const IPoint& ip) { return 7 * ip.m_x + 23 * ip.m_y; }
};
...
bool GridLayout::checkLayout() {
	...
	for (edge e : G.edges) {
		...
		for (const IPoint& ip : bends) {
			GridPointInfo i = H[ip];
			if (i != GridPointInfo()) {
				std::cout << "conflict of bend point " << ip << " of edge " << e << " with "
						  << H[ip] << std::endl;
				return false;
			}
...
GridLayoutMapped::GridLayoutMapped(const PlanRep& PG, const OrthoRep& OR, double separation,
		double cOverhang, int fineness)
