...
#define forallnodesonlayer for (actNode = first[actLayer]; actNode < first[actLayer + 1]; actNode++)
...
public:
	...
	void reduce(kList& newList) {
		...
		while (!empty()) {
			oldWK = popFrontRet();
			if (oldWK.key > newWK.key + ALLOW || oldWK.key < newWK.key - ALLOW) {
				if (newWK.element) {
...
void FastHierarchyLayout::doCall(const HierarchyLevelsBase& levels, GraphAttributes& AGC) {
		...
		if (e1 && GC.chain(e1).size() > 1 && e == GC.chain(e1).front()) {
			...
			for (edge e2 : GC.chain(e1)) {
				...
				int n1 = first[H.rank(v1)]
						+ levels.pos(v1); // Number nodes top down and from left to right
				newEdge->pushBack(n1);
			}
...
void FastHierarchyLayout::sortLongEdges(int actNode, int dir, double* pos, bool& exD, double& dist,
	...
	for (int next : *longEdge[actNode]) {
		if (sameLayer(next - dir, next) && block[next - dir] == block[next]) {
			...
			if (!exB || dir * (best - pos[next - dir]) < dir * (totalB[next] - totalB[next - dir])) {
				exB = 1;
	...
	for (int next : *longEdge[actNode]) {
		...
		if (sameLayer(next + dir, next) && block[next + dir] != block[next]
				&& (!exD
						|| dir * (totalB[next + dir] - totalB[next] - pos[next + dir] + pos[next])
								> dist)) {
			dist = dir * (totalB[next + dir] - totalB[next] - pos[next + dir] + pos[next]);
			exD = 1;
		}
	}
}

bool FastHierarchyLayout::placeSingleNode(int leftBnd, int rightBnd, int actNode, double& best,
		int d) {
	kList neighbours;
...
void FastHierarchyLayout::placeNodes(int leftBnd, int rightBnd, int left, int right, int d) {
	if (left == right) {
					...
					if (dir * (x[next] - x[actNode]) < ALLOW) {
						...
						if (dir * (x[mdl[i]] - newBend) > -ALLOW) {
							res[i]++;
								...
								(bnd[i] == -1
										|| dir * (newBend - x[bnd[i]] + mDist[bnd[i]] - mDist[mdl[i]])
												< ALLOW)
		...
		actNode = mdl[0];
		while (--actNode >= left && x[mdl[0]] - x[actNode] < mDist[mdl[0]] - mDist[actNode]) {
			x[actNode] = x[mdl[0]] - mDist[mdl[0]] + mDist[actNode];
		}
		actNode = mdl[1];
		while (++actNode <= right && x[mdl[1]] - x[actNode] > mDist[mdl[1]] - mDist[actNode]) {
			x[actNode] = x[mdl[1]] - mDist[mdl[1]] + mDist[actNode];
		}
	}
}

void FastHierarchyLayout::moveLongEdge(int actNode, int dir, bool* marked) {
		...
		while (!done && !toTest.empty()) {
			...
			for (int next : *longEdge[actNode]) {
				...
				if (!isFirst(next)) {
					...
					done = done && xOpt - x[next - 1] >= totalB[next] - totalB[next - 1] - ALLOW;
				}
...
void FastHierarchyLayout::straightenEdge(int actNode, bool* marked) {
	if (!marked[actNode] && // breadth[actNode] < ALLOW &&
			adj[0][actNode].size() == 1 && adj[1][actNode].size() == 1
		...
		double xOpt = x[fst]
				+ (x[lst] - x[fst]) * (y[layer[actNode]] - y[layer[fst]])
...
void FastHierarchyLayout::findPlacement() {
	...
	// nonvirtual node.
	forallnodes if (virt[actNode] && !virt[adj[0][actNode].front()]
			&& !virt[adj[1][actNode].front()]) virt[actNode] = 0;
	...
	double toAdd;
	forallnodes {
		if (!actNode || layer[actNode - 1] < layer[actNode]) {
			totalB[actNode] = 0;
		} else {
			toAdd = breadth[actNode - 1] / 2 + breadth[actNode] / 2;
			// Enlarge the minimal Distance for nodes with a lot of neighbours.
			incrTo(toAdd,
					m_minNodeDist / 3 * (double)(adj[0][actNode - 1].size() + adj[0][actNode].size()));
			incrTo(toAdd,
					m_minNodeDist / 3 * (double)(adj[1][actNode - 1].size() + adj[1][actNode].size()));
			...
			if (virt[actNode]) {
				...
				if (virt[down]) {
					if (last != -1 && last > down) {
						...
						oldEdge->split(oldEdge->get(spl), (*newEdge), (*oldEdge));
	...
	for (dir = 1; dir >= -1; dir -= 2) {
		...
		int blockCount = 0;
		forallnodes block[actNode] = marked[actNode] = 0;
		foralllayers {
		...
		foralllayers {
			for (actNode = ((dir == 1) ? first[actLayer] : (first[actLayer + 1] - 1));
					actNode >= first[actLayer] && actNode < first[actLayer + 1]; actNode += dir) {
		...
		// and right placement.
		forallnodes x[actNode] += pos[actNode] / 2;
	...
	forallnodes mDist[actNode] = totalB[actNode];

	for (dir = 0; dir < 2; dir++) {
		// for every traversal

		for (actLayer = dir ? k - 1 : 0; 0 <= actLayer && actLayer < k; actLayer += dir ? -1 : 1) {
			...
			forallnodesonlayer {
				if (virt[actNode]) {
					...
					} else if (nodeDir[leftBnd]
							!= !dir) { // nodeDir[leftBnd] == dir ||  nodeDir[leftBnd] == -1
						// internal sequence
	...
	forallnodes marked[actNode] = 0;
	foralllayers {
		for (actNode = (first[actLayer] + first[actLayer + 1]) / 2; actNode < first[actLayer + 1];
				actNode++) {
			moveLongEdge(actNode, 0, marked);
		}
		for (actNode = (first[actLayer] + first[actLayer + 1]) / 2 - 1; actNode >= first[actLayer];
				actNode--) {
			moveLongEdge(actNode, 0, marked);
		}
	}
